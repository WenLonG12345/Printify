...
const App: React.FC = () => {
  ...
  const addNote = (text: string, color: NoteData['color'], startX?: number, startY?: number) => {
    const id = Date.now().toString();
    const rotation = Math.random() * 8 - 4; 
    
    const stickers: NoteData['sticker'][] = [
      'heart', 'sparkles', 'star', 'smile', 
      'sun', 'moon', 'cloud', 'music', 
      'zap', 'ghost', 'flower'
    ];
    const randomSticker = stickers[Math.floor(Math.random() * stickers.length)];

    // Spawn logic
    const spawnX = startX ?? (window.innerWidth / 2) - 120 + (Math.random() * 40 - 20);
