...
import { 
  Download, X, 
  Heart, Sparkles, Star, Smile,
  Sun, Moon, Cloud, Music, Zap, Ghost, Flower
} from 'lucide-react';
...
const StickerWrapper = ({ children, colorClass }: React.PropsWithChildren<{ colorClass: string }>) => (
...
const StickerIcon = ({ type }: { type: NoteData['sticker'] }) => {
  const commonClass = "w-full h-full fill-current";

  switch (type) {
    // Solid Shapes (Stroke 0)
    case 'heart': 
      return <StickerWrapper colorClass="text-red-500"><Heart className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'sparkles': 
      return <StickerWrapper colorClass="text-yellow-400"><Sparkles className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'star': 
      return <StickerWrapper colorClass="text-orange-400"><Star className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'moon':
      return <StickerWrapper colorClass="text-indigo-400"><Moon className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'cloud':
      return <StickerWrapper colorClass="text-sky-400"><Cloud className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'music':
      return <StickerWrapper colorClass="text-violet-500"><Music className={commonClass} strokeWidth={0} /></StickerWrapper>;
    case 'zap':
      return <StickerWrapper colorClass="text-yellow-500"><Zap className={commonClass} strokeWidth={0} /></StickerWrapper>;
    
    // Line Art / Detailed Shapes (Stroke 2.5)
    case 'smile': 
      return <StickerWrapper colorClass="text-blue-500"><Smile className={commonClass} strokeWidth={2.5} /></StickerWrapper>;
    case 'sun':
      return <StickerWrapper colorClass="text-orange-500"><Sun className={commonClass} strokeWidth={2.5} /></StickerWrapper>;
    case 'ghost':
      return <StickerWrapper colorClass="text-slate-400"><Ghost className={commonClass} strokeWidth={2.5} /></StickerWrapper>;
    case 'flower':
      return <StickerWrapper colorClass="text-pink-500"><Flower className={commonClass} strokeWidth={2.5} /></StickerWrapper>;
      ...
      <div className="animate-print">
        ...
        >
          ...
          <div className="relative p-6 pt-10 flex-1 flex flex-col items-center">
            ...
            {/* Content */}
            <div className="font-hand text-2xl leading-relaxed text-center w-full mb-6 break-words text-gray-800/90" 
            ...
            <div className="mt-auto w-full flex justify-center">
              <span className="text-[9px] font-bold text-black/20 uppercase tracking-[0.2em]">
                Printify • {new Date(parseInt(data.timestamp)).toLocaleDateString()}
              </span>
            </div>
          </div>
        </div>
