      ...
      "
			>
				...
				<div className="relative group">
					...
					<div className="relative bg-white rounded-xl p-1">
						<textarea
							...
							className="w-full h-24 bg-transparent border-none focus:ring-0 text-slate-700 text-lg placeholder-slate-400 resize-none leading-relaxed p-3 outline-none font-medium rounded-lg"
							maxLength={120}
						/>
				...
				<div className="flex items-center justify-between">
					{/* Color Palette */}
					<div className="flex items-center gap-2 bg-slate-100/80 p-2 rounded-full">
						{colors.map((c) => (
							<button
								key={c.id}
								type="button"
								...
								className={`
                            w-5 h-5 rounded-full transition-all duration-300 border
                            ${c.class}
                            ${
															selectedColor === c.id
                    ...
                    `}
						>
							<Sparkles
								size={14}
								className={`text-purple-500 ${isGenerating ? "animate-spin" : ""} ${isLimitReached ? "grayscale" : ""}`}
							/>
                        ...
                        transition-all duration-200
                        ${!text.trim() ? "opacity-50 cursor-not-allowed" : "hover:bg-black hover:scale-105"}
                    `}
						>
							<Send
								size={16}
